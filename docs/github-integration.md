...
model ProductRepository {
  ...
  owner                String    // e.g., "linuxfoundation"
  name                 String    // e.g., "lfx-changelog"
  fullName             String    // e.g., "linuxfoundation/lfx-changelog"
  htmlUrl              String
  description          String?
...
model GitHubRelease {
  id              String    @id @default(uuid())
  repositoryId    String
  githubId        Int       // GitHub's release ID
...
Multiple server replicas may receive webhooks simultaneously. A database-based distributed lock prevents duplicate generation:

```prisma
model AutoChangelogLock {
  productId String                  @id
  status    AutoChangelogLockStatus // 'in_progress' or 'pending_rerun'
  lockedAt  DateTime
  updatedAt DateTime
}
```
...
enum ChangelogSource {
  manual      // Created by a human user
  automated   // Generated by the AI from GitHub activity
}
```
