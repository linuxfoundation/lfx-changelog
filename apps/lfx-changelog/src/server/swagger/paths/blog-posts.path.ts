...
blogPostRegistry.registerPath({
  ...
  responses: {
    200: {
      ...
      content: { 'application/json': { schema: createPaginatedResponseSchema(BlogPostSchema) } },
    },
...
blogPostRegistry.registerPath({
  method: 'post',
  path: '/api/blog-posts/{id}/link-products',
  tags: ['Blog Posts'],
...
blogPostRegistry.registerPath({
  method: 'post',
  path: '/api/blog-posts/{id}/link-changelogs',
  tags: ['Blog Posts'],
  summary: 'Link changelog entries to blog post',
  description: 'Replaces the changelog entry associations for a blog post.\n\n**Required privilege:** PRODUCT_ADMIN role or above.',
  security: API_KEY_AUTH,
  request: {
    params: z.object({ id: z.string().openapi({ description: 'Blog post ID' }) }),
