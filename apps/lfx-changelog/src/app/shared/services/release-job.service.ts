...
export class ReleaseJobService {
  ...
  public start(serviceKey: string, notes: string, newTag?: string): Observable<ReleaseJob> {
    return this.http.post<ApiResponse<ReleaseJob>>('/api/release-jobs', { serviceKey, notes, newTag }).pipe(
      map((res) => res.data),
      take(1)
    );
  }
