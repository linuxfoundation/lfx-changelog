...
import { catchError, firstValueFrom, map, of, Subject, Subscription, switchMap } from 'rxjs';
...
export class ChatService {
  ...
  public sendMessage(message: string): void {
    ...
    this.subscription = this.sseService
      .connect<ChatSSEEventType>(url, {
        method: 'POST',
        body: { conversationId: this.conversationId() ?? undefined, message },
      })
      .subscribe({
        next: (event) => this.handleSSEEvent(event as { type: ChatSSEEventType; data: string }),
        error: (err) => {
          this.flushCharBuffer();
          this.error.set(`Connection error: ${err instanceof Error ? err.message : 'Unknown error'}`);
