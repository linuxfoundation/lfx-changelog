...
export function registerChangelogTools(server: McpServer, apiClient: ApiClient): void {
  server.registerTool(
    ...
    async ({ productId, page, limit }) => {
      try {
        ...
        return jsonResult(await apiClient.get('/public/api/changelogs', query));
      } catch (error) {
        return errorResult('Error fetching changelogs', error);
      }
    }
  );
  ...
  server.registerTool(
    ...
    async ({ id }) => {
      try {
        return jsonResult(await apiClient.get(`/public/api/changelogs/${encodeURIComponent(id)}`));
      } catch (error) {
        return errorResult('Error fetching changelog', error);
      }
    }
  );
}
